...
class MCG(object):
    ...
    def __init__(self, n_comm=2, game=None, n_acts=2, mem_size=0, adv_coeff=0, payoffs_a=None, payoffs_b=None, ob_ma=None, act_re=None):
        """
        ...
        # todo modify
        self.n_obs = 2 * (self.n_acts ** 2) + self.n_comm * 2 + self.n_acts * 2 + (self.n_comm + self.n_acts) * 2 * self.mem_size

        # Initialize agent memory
        self.mem = deque([np.zeros(2 * (self.n_comm + self.n_acts))] * self.mem_size, self.mem_size)

        # todo add new game settings
        # Initialize the setting whether one can observe all payoff-matrices and whether reveal its action
        self.ob_ma = ob_ma
        self.act_re = act_re
    ...
    def build_state(self, new_mats=True):
        ...
        state = [np.concatenate([*list(self.mem), self.payoff_mat_a.flatten(), self.payoff_mat_b.flatten(),
                                self.comm_a, self.act_a, self.comm_b, self.act_b])] * 2

        # todo add
        # Set 0 in corresponding part of state according to the setting of the game
        if not self.act_re:
            # Mask opponent's committed action and communication(i.e. whether to reveal its action)
            state[0][-(self.n_acts+self.n_comm): ] = 0
            state[1][-2 * (self.n_acts+self.n_comm): -(self.n_acts+self.n_comm)] = 0

        if not self.ob_ma:
            # Mask opponent's payoff-matrix
            state[0][-2 * (self.n_acts+self.n_comm)-self.n_acts*self.n_acts: -2 * (self.n_acts+self.n_comm)] = 0
            state[1][-2 * (self.n_acts+self.n_comm)-2*self.n_acts*self.n_acts:
                   -2 * (self.n_acts+self.n_comm)-self.n_acts*self.n_acts] = 0
    ...
    def step_a_single(self, action, ag, garble=False):
        # Update the state based on a single agent's action
        if ag == 0:
            self.act_a = U.index_to_onehot(action, v_len=self.n_acts)
        elif ag == 1:
            self.act_b = U.index_to_onehot(action, v_len=self.n_acts)
        return self.build_state(new_mats=False)
