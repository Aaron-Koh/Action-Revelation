...
def parse_args():
    ...
    parser.add_argument("--n-comm", type=int, default=2, help="size of agent vocabulary")
    parser.add_argument("--game", type=str, default="specfixed", help="type of coordination game being played")
    ...
    # todo set whether one can observe all payoff-matrices
    parser.add_argument("--ob-ma", action='store_true', default=False, help='if True, one can observe all payoff-matrices')
    # todo set whether there exists action revelation
    parser.add_argument('--act-re', action='store_true', default=False, help='if True, one can commit its action to its opponet')
...
def train(args):
    ...
    # Scale communication channel and MLP size based on size of game (unless set_manual_n_hid = True)
    # todo anotate and modify
    ...
    env = MCG(n_comm=args.n_comm, game=args.game, n_acts=args.n_acts, mem_size=args.mem_size,\
              adv_coeff=args.adv_coeff, ob_ma=args.ob_ma, act_re=args.act_re)
    ...
    if args.exp_name is None:
        ...
        # args.exp_name += f'_cc={args.c_coeff}_gnc={args.gn_coeff}_gam={args.gamma}_nh={args.n_hid}' # todo
        ...
        # args.exp_name += f'_hnoise={args.hnoise_sigma}_advc={args.adv_coeff}_maskc={args.mask_c_learning}' # todo
        ...
        with torch.no_grad():
            if args.comm_type == "turns" or args.comm_type == "garble":
                ...
                if c1 == 1 and c2 == 1:
                    ...
                    if j == 0:
                        c2 = 0
                        ob_a1 = env.step_c_single(c1, j, garble=args.comm_type == 'garble')
                        a1 = agent[0].act(torch.Tensor(ob_a1[0]), mode='act')
                        ob_a2 = env.step_a_single(a1, j, garble=args.comm_type == 'garble')
                        a2 = agent[1].act(torch.Tensor(ob_a2[1]), mode='act')
                        ob_a = [ob_a1[0], ob_a2[1]]
                    else:
                        c1 = 0
                        ob_a2 = env.step_c_single(c2, j, garble=args.comm_type == 'garble')
                        a2 = agent[1].act(torch.Tensor(ob_a2[1]), mode='act')
                        ob_a1 = env.step_a_single(a2, j, garble=args.comm_type == 'garble')
                        a1 = agent[0].act(torch.Tensor(ob_a1[0]), mode='act')
                        ob_a = [ob_a1[0], ob_a2[1]]
                elif c1 == 1 and c2 == 0:
                    # 1.2 如果 agent0 决定 commit action e.g. c1 = 1, c2 = 0
                    ob_a1 = env.step_c_single(c1, 0, garble=args.comm_type == 'garble')
                    a1 = agent[0].act(torch.Tensor(ob_a1[0]), mode='act')
                    ob_a2 = env.step_a_single(a1, 0, garble=args.comm_type == 'garble')
                    a2 = agent[1].act(torch.Tensor(ob_a2[1]), mode='act')
                    ob_a = [ob_a1[0], ob_a2[1]]
                elif c1 == 0 and c2 == 1:
                    # 1.3 如果 agent1 决定 commit action e.g. c1 = 0, c2 = 1
                    ob_a2 = env.step_c_single(c2, 1, garble=args.comm_type == 'garble')
                    a2 = agent[1].act(torch.Tensor(ob_a2[1]), mode='act')
                    ob_a1 = env.step_a_single(a2, 1, garble=args.comm_type == 'garble')
                    a1 = agent[0].act(torch.Tensor(ob_a1[0]), mode='act')
                    ob_a = [ob_a1[0], ob_a2[1]]
                else:
                    # 1.4 双方都不 commit action，那么双方同时执行动作
                    a1 = agent[0].act(torch.Tensor(ob_c[0]), mode='act')
                    a2 = agent[1].act(torch.Tensor(ob_c[1]), mode='act')
                    ob_a = [ob_c[0], ob_c[1]]
                ...
                c1 = agent[0].act(torch.Tensor(ob_c[0]), mode='comm')
                ob_a1 = env.step_c_single(c1, 0)
                a1 = agent[0].act(torch.Tensor(ob_a1[0]), mode='act')
                ob_a2 = env.step_a_single(a1, 0)
                a2 = agent[1].act(torch.Tensor(ob_a2[1]), mode='act')
                c = [c1, 0]
                a = [a1, a2]
                ob_a = [ob_a1[0], ob_a2[1]]
        ...
        if len(acts[0]) == args.batch_size:
            ...
            for j in range(2):
                ...
                if j == 0:
                    ...
                    writer.add_scalar('Agent1/sc', eval.calc_mutinfo(acts[j], comms[j], args.n_acts, args.n_comm), n_batch)
                    writer.add_scalar('Agent1/ic', eval.calc_mutinfo(acts[1 - j], comms[j], args.n_acts, args.n_comm), n_batch)
